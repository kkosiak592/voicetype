...
export function FillerRemovalToggle() {
  const [enabled, setEnabled] = useState(false);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    // Read from settings store (safe before setup — returns null/false).
    // Avoids calling get_filler_removal which accesses ActiveProfile state
    // that isn't manage()'d until deep in setup().
    store.get<boolean>('filler_removal').then((val) => {
  ...
  async function handleToggle() {
    const next = !enabled;
    await invoke('set_filler_removal', { enabled: next });
    setEnabled(next);
  }
  ...
  return (
    ...
    >
      <span className="sr-only">Toggle filler word removal</span>
      <span
        aria-hidden="true"
        className={[
