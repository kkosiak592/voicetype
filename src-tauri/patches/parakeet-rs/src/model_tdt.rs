...
impl Default for TDTModelConfig {
    fn default() -> Self {
        Self {
            // Placeholder — from_pretrained() always receives vocab_size from vocab.txt.
            // This default is not used in normal operation.
            vocab_size: 0,
        }
    }
}
...
impl ParakeetTDTModel {
    ...
    ///
    /// `vocab_size` must be provided by the caller (derived from vocab.txt line count).
    /// This is the only reliable source for vocab_size — the downloaded config.json does
    /// not contain a vocab_size field for the parakeet-tdt-0.6b-v2-onnx int8 model.
    ///
    /// Using the wrong vocab_size causes blank_id = vocab_size - 1 to be out of the
    /// ONNX embedding table range, triggering a Gather node OOB error on the first
    /// decoder_joint call.
    pub fn from_pretrained<P: AsRef<Path>>(
        ...
        // Use vocab_size from caller (derived from vocab.txt — the ground truth).
        // config.json in the downloaded parakeet-tdt-0.6b-v2-onnx model does NOT contain
        // a vocab_size field, so reading it would silently fall back to the wrong default.
        let config = TDTModelConfig { vocab_size };
        ...
        eprintln!(
            "[parakeet-rs] Sessions created with {} ExecutionProvider (vocab_size={})",
            exec_config.provider_name(),
            vocab_size,
        );
    ...
    fn greedy_decode(&mut self, encoder_out: &Array3<f32>, encoder_len: i64) -> Result<(Vec<usize>, Vec<usize>, Vec<usize>)> {
        // encoder_out shape: [batch, encoder_dim, time]
        let encoder_dim = encoder_out.shape()[1];
        // Use valid frame count from encoder, not padded tensor length
        let time_steps = (encoder_len as usize).min(encoder_out.shape()[2]);
        ...
        while t < time_steps {
            ...
            // TDT outputs vocab_size + 5 durations (e.g. 1025 + 5 = 1030 for int8 model)
