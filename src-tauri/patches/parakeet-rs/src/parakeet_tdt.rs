...
impl ParakeetTDT {
    ...
    pub fn from_pretrained<P: AsRef<Path>>(
        ...
        // Load vocab BEFORE model so vocab_size is available as the ground truth.
        // The downloaded config.json does not contain a vocab_size field, so we must
        // derive it from the vocabulary file (vocab.txt line count = number of tokens).
        let vocab = Vocabulary::from_file(&vocab_path)?;
        let vocab_size = vocab.id_to_token.len();
        eprintln!(
            "[parakeet-rs] vocab loaded: {} tokens, blank_id={}",
            vocab_size,
