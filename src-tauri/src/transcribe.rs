...
///
/// Always runs with no_context=true to prevent context carryover between recordings.
///
...
pub fn transcribe_audio(ctx: &WhisperContext, audio: &[f32]) -> Result<String, String> {
    ...
    params.set_no_context(true);   // no prior context carryover between recordings
